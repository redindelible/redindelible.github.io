...
difficult to get working properly, and so are even harder to learn about. 
...
To increase the learning we derive from this exercise, no tools beyond the standard library of our host language will be 
used. This means that no parsing libraries will be used, nor any assembly-generation tools like LLVM. Although it is more 
practical in real life to use such tools, the purpose of this series is learning, which will be necessarily held back 
should shortcuts be used.

Specifically, the compiler here will be written in Python, for two main reasons. One, Python is a very popular and comprehensible 
programming language, so this series has a better chance of being understandable compared to Haskell or Rust. Second, 
Python code tends to have a lot less boilerplate compared to something like C or Java, making reading and writing it much
easier.